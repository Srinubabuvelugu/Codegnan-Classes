...
def recentNotes(user_id):
    try:
        ...
        if connection=='Connection Failed':
            ...
            get_recetn_notes = """SELECT * FROM NOTES
                                WHERE USER_ID = %s order by updated_at desc limit 3;"""
            cursor.execute(get_recetn_notes,(user_id,))
            recent_notes= cursor.fetchall()
            
            cursor.close()
            connection.close()
            return True, recent_notes
    except Exception as e:
        return False, f"Execption raised in add notes: {e}"
...
def dashboard():
    ...
    if "id" not in session:
        return redirect("/login")
    userid = session['id']
    notes_count, files_count = getFilesAndNotesCount(userid)
    _, recent_notes = recentNotes(userid)
    print(recent_notes)
...
def addNotesRecord(user_id:int, title:str, content:str):
    try:
        ...
        if connection=='Connection Failed':
            ...
            cursor=connection.cursor()
            add_notes_query = """INSERT INTO NOTES(user_id, title, content)
                                    values(%s, %s, %s)"""
...
def GetFilesbyUserid(userid, filename=None):
    try:
        connection=getConnectionWithDB()
        if connection=='Connection Failed':
            return False, "Database connection Failed"
            ...
            values = [userid]
            if filename:
                query += f" and stored_name like %s;"
                values.append(f"%{filename}%")
            get_file_by_userid = """select * from file_data""" + query
            print(get_file_by_userid)
            cursor.execute(get_file_by_userid,tuple(values))
            print(cursor)
        ...
        return False, f"Execption raised in get Files: {e}"
...
def myfiles():
    ...
    if request.method == 'GET':
        # get all files
        searchfile = request.args.get('searchfiles', None)
        print(searchfile)
        userid = session['id']
        status, files = GetFilesbyUserid(userid=userid,filename=searchfile)
        # print(files)
        if status== True:
